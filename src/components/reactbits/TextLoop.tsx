...
const buildPath = (shape: string, curviness: number, ribbonWidth: number): string => {
  ...
  switch (shape) {
    case 'circle': {
      const r = Math.min(90 + c * 0.8, room);
      return `M ${CX - r} ${CY} A ${r} ${r} 0 1 1 ${CX + r} ${CY} A ${r} ${r} 0 1 1 ${CX - r} ${CY} Z`;
    }
    case 'infinity': {
      const r = 160 + c * 1.2;
      const h = Math.min(60 + c * 0.8, room);
      return [
        `M ${CX} ${CY}`,
    ...
    case 'arch': {
      const rise = Math.min(100 + c * 1.0, room * 2);
    ...
    case 'wave':
    default: {
      const a = Math.min(c * 1.4, room * 0.95);
      return `M -400 ${CY} Q -200 ${CY - a} 0 ${CY} T 200 ${CY} T 400 ${CY} T 600 ${CY} T 800 ${CY} T 1000 ${CY} T 1200 ${CY} T 1400 ${CY} T 1600 ${CY} T 1800 ${CY} T ${VIEW_W + 400} ${CY}`;
    }
  }
};
