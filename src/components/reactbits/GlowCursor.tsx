...
export const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      canvas,
      alpha: true,
      dpr: Math.min(window.devicePixelRatio || 1, initialConfig.maxDevicePixelRatio || 1.5)
    });
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPointCount: { value: initialConfig.trailLength || 40 },
        uColor: { value: hexToRgb(initialConfig.color || '#67E8F9') },
        uSecondaryColor: { value: hexToRgb(initialConfig.secondaryColor || '#A78BFA') },
        uTrailWidth: { value: initialConfig.trailWidth || 8 },
        uTaper: { value: initialConfig.trailTaper || 0.8 },
        uGlowIntensity: { value: initialConfig.glowIntensity || 1.9 },
        uGlowSpread: { value: initialConfig.glowSpread || 1.2 },
        uHotspot: { value: initialConfig.hotspot || 0.65 },
        uBrightness: { value: initialConfig.brightness || 1.25 },
        uOpacity: { value: initialConfig.opacity || 1 },
        uPulseSpeed: { value: initialConfig.pulseSpeed || 1.1 },
        uNoiseStrength: { value: initialConfig.noiseStrength || 0.035 },
    ...
    const render = (now: number) => {
      ...
      if (initialized) {
        const headEase = 1 - Math.pow(1 - clamp(config.followSpeed || 0.16, 0.01, 0.99), delta);
        const chainBase = clamp(0.28 + (config.followSpeed || 0.16) * 0.35, 0.08, 0.92);
      ...
      const shouldFade = config.idleFade && (!pointerInside || idleFor > (config.idleTimeout || 700));
      const fadeStep = (16.667 * delta) / Math.max(config.fadeDuration || 900, 16);
  ...
  return (
    <div ref={containerRef} className={`glow-cursor${className ? ` ${className}` : ''}`} style={style} {...rest}>
      <canvas
        ...
        style={{ mixBlendMode: (blendMode as any) || 'screen' }}
        aria-hidden="true"
      />
